...
- [ ] `POSTMARK_SERVER_TOKEN` — **(04/09/2026, pas encore configuré)** Server API Token Postmark (sortant — différent de `POSTMARK_WEBHOOK_SECRET`, qui sécurise seulement le webhook entrant) nécessaire au bouton "Envoyer (test)" des rappels (`_shared/email.ts`, `secure-data:rappels_envoyer_test`) : envoie le lien de rappel par email pour tester le parcours patient avant que le SMS réel soit branché. Sans lui, le bouton échoue proprement avec un message explicite (vérifié en direct). À récupérer dans le même compte Postmark que l'entrant (Servers → le serveur → API Tokens). Prévoir aussi `EMAIL_FROM` (adresse expéditrice vérifiée dans Postmark, ex. `OrdoMail <notifications@ordomail.fr>`) — sinon un défaut non vérifié est utilisé et Postmark rejettera l'envoi.
